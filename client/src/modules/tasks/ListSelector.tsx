...
export function ListSelector({
  ...
  const handleCreateList = () => {
    ...
    if (name && activeHouseholdId) {
      onCreateList(name, activeHouseholdId);
      setNewListName('');
      setCreating(false);
      setOpen(false);
    }
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-1 text-[15px] font-semibold text-gray-900 dark:text-gray-100 transition-colors px-2 py-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 max-w-full"
      >
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50 animate-fadeIn max-h-[70dvh] overflow-y-auto">
          {lists.length === 0 && (
            <p className="px-3 py-2 text-sm text-gray-400">Brak list w tym gospodarstwie.</p>
          )}
