...
// Live updates for the finance module (wallets/transactions), scoped per
// household (room `household:<id>`).
@WebSocketGateway({
  namespace: 'finance',
  cors: {
    origin: true,
...
export class FinanceGateway implements OnGatewayConnection {
  @WebSocketServer()
  server!: Server;
  ...
  notifyChanged(householdId: string): void {
    this.server.to(`household:${householdId}`).emit('finance:changed', { householdId });
  }
