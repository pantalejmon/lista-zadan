...
// Live updates for the home-service module (assets/maintenance), scoped per
...
export class HomeGateway implements OnGatewayConnection {
  @WebSocketServer()
  server!: Server;
  ...
  notifyChanged(householdId: string): void {
    this.server.to(`household:${householdId}`).emit('home:changed', { householdId });
  }
